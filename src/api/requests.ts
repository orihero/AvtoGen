...
export let configureAxios = storeInstance => {
	axios.interceptors.request.use(req => {
		req.headers = {
			...
			"Accept-Language": storeInstance.getState().user.settings.language
		};
		return req;
	});
};
...
let requests = {
	...
	main: {
		companies: (id = 1) =>
			axios.get(`${URL}/hand/companies?category_id=${id}`),
		services: () => axios.get(`${URL}/hand/services`),
		carTypes: () => axios.get(`${URL}/hand/car-types`),
		searchCompanies: data =>
			axios.post(`${URL}/hand/search-company`, formData(data)),
		book: credentials =>
			axios.post(`${URL}/booking/book`, formData(credentials)),
		books: status =>
			axios.get(`${URL}/booking/client-books?status=${status}`),
		cancel: id => axios.get(`${URL}/booking/reject-book/${id}`),
		setBookingState: (id, status) =>
			axios.post(
				`${URL}/booking/set-status-book`,
				formData({ booking_id: id, status })
			),
		comment: credentials =>
			axios.post(`${URL}/booking/comment-book`, formData(credentials))
	},
	user: {
	...
	profile: {
		setToken: credentials =>
			axios.post(`${URL}/profile/setFcmToken`, formData(credentials))
	}
};
